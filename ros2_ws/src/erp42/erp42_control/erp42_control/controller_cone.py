    ...
    def publish_cmd(self):
        ...
        if self.first_lap: # first lap
            if self.decision_last_idx(target_idx):
                ...
                adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=1, max_value=3, he_gain=50.0, ce_gain=30.0, he_thr=0.001, ce_thr=0.002)
                ...
                if self.decision_straight(target_idx):
                    ...
                    adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=4, max_value=7, he_gain=40.0, ce_gain=30.0, he_thr=0.07, ce_thr=0.05)
                    if self.state.v * 3.6 >= adapted_speed:
                        input_brake = (abs(self.state.v * 3.6 - adapted_speed) / 20.0) * 200
                    else:
                        input_brake = 0
            ...
            if self.decision_straight(target_idx):
                ...
                steer, hdr, ctr = self.st.stanley_control(self.state, self.path.cyaw, h_gain_straight, c_gain_straight, target_idx, error)
                adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=8, max_value=13, he_gain=40.0, ce_gain=20.0, he_thr=0.07, ce_thr=0.05)
                if self.state.v * 3.6 >= adapted_speed:
                    input_brake = (abs(self.state.v * 3.6 - adapted_speed) / 20.0) * 200
                else:
                    input_brake = 0
                ...
                steer, hdr, ctr = self.st.stanley_control(self.state, self.path.cyaw, h_gain_curve, c_gain_curve, target_idx, error)
                adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=5, max_value=7, he_gain=50.0, ce_gain=30.0, he_thr=0.001, ce_thr=0.002)
                if self.state.v * 3.6 >= adapted_speed:
                    input_brake = (abs(self.state.v * 3.6 - adapted_speed) / 20.0) * 200
                else:
                    input_brake = 0
