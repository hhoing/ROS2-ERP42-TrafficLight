...
class PID():
    ...
    def PIDControl(self, speed, desired_value):
        ...
        return int(np.clip(self.speed, 5, 8))
...
class Stopline():
    ...
    def control_stop_line(self, odometry, path):
        ...
        if self.target_idx >= len(path.cx) - 5: # distance가 0.5m 이내
            ...
            self.target_speed = int(np.clip(self.target_speed, 5, 8))
            adapted_speed = self.ss.adaptSpeed(self.target_speed, hdr, ctr, min_value=5, max_value=8)
