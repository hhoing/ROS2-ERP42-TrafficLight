    ...
    def update_cmd_msg(self):
        ...
        if self.state.value[:-2] == "driving":
            if self.odometry.x != 0.: #10.03 수정
                ...
                speed = self.pid.PIDControl(self.odometry.v * 3.6, adapted_speed, min=4, max=15) # speed 조정 (PI control) 
            ...
            if self.odometry.x != 0.: #10.03 수정
                steer, self.target_idx, hdr, ctr = self.st.stanley_control(self.odometry, self.path.cx, self.path.cy, self.path.cyaw, h_gain=0.5, c_gain=0.24)
                target_speed = self.set_target_speed()
                adapted_speed = self.ss.adaptSpeed(target_speed, hdr, ctr, min_value=4, max_value=8) # 에러(hdr, ctr) 기반 목표 속력 조정
                speed = self.pid.PIDControl(self.odometry.v * 3.6, adapted_speed,  min=4, max=8) # speed 조정 (PI control) 
                brake = self.cacluate_brake(adapted_speed) # brake 조정
            ...
            if self.k < 1:
                try:
                    set_param("bs_cropbox_filter","detection_area","[0.0, 20.0, -2.5, 2.5]")
...
def main():
    ...
    node.declare_parameter("file_name", "YS_1020.db")
