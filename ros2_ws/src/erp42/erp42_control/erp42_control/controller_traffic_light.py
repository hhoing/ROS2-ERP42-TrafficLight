...
class Trafficlight:
    def __init__(self, node):
        ...
        self.traffic_light_sections = [
            ...
            (
                {"1401", "1402"},           # 첫 번째 신호등 (current_index=0) 
                {"1405"},
            ),                             
            ({"1401", "1402"}, {"1405"}),  # 두 번째 신호등 (current_index=1)
            ({"1401"}, {"1400", "1405"}),  # 세 번째 신호등 (current_index=2)
            (
                {"1400", "1401", "1402", "1404"},
                {"1403"},
            ),                              # 네 번째 신호등 (current_index=3)
            ({"1301"}, {"1303"}),           # 다섯 번째 신호등 (current_index=4)
            ({"1401", "1402"}, {"1405"}),  # 여섯 번째 신호등 (current_index=5)
            ({"1401", "1402"}, {"1405"}),  # 일곱 번째 신호등 (current_index=6)
        ] 
    ...
    def control_traffic_light(self, odometry, path):
        ...
        if self.current_signal == "red":
            if current_distance_to_stop_line <= 10:  # 남은 거리 1m이하일때
                speed = 0  # 속도 0으로 설정하여 정지
                estop = 1  # 비상 정지 설정
                self.node.get_logger().info(f"빨간불 확인: 차량 정지, 현재속도 = {msg.speed}")


                if time.time() - self.last_signal_time > 3:
                    self.node.get_logger().info("신호없름,그냥출발")
                    self.red_light_detected =False
                    self.green_light_detected = False
                    self.current_signal = None
            ...
            # 빨간불인데 서서히 속도 감속
                ...
                self.target_speed = int(np.clip(self.speed, 4, 6))
                adapted_speed = self.ss.adaptSpeed(self.target_speed, hdr, ctr, min_value=4, max_value=8)
        ...
        # 2. 초록불 감지된 경우 처리
            ...
            # 일정 거리 후 미션 종료 조건 추가
            if current_distance_to_stop_line <= 5:  # 정지선 근처에서 미션 완료 처리
        ...
        # 신호등 미션 완료되었을 때, 인덱스 증가 및 신호정보 초기화
        if self.mission_finish:
            self.current_index += 1  # 다음 신호등 구간으로 이동
            self.reset_traffic_light()  # 신호 상태 초기화
