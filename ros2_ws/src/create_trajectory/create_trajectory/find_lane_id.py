...
class Find_lane_id(Node):
    def __init__(self):
        super().__init__("current_id") # method overloading
    ...
    def lane_callback(self,msg):
        lanes = []
        for i in msg.points[0:10]: #### 왜 5번밖에 반복 안하지??????????/
            # print(i.lane_ids)
            lane_number = len(i.lane_ids)
            # if lane_number == 1:
            #     lanes.append(i.lane_ids[0]) #lane_ids of each point
            # elif lane_number == 0:
            #     c +=1
            #     print("empty_lane: ",c)
            # else:
