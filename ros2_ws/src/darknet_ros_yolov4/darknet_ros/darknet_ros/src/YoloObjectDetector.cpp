      ...
      preempt_requested_(false)
{
  ...
  declare_parameter("yolo_model.weight_file.name", std::string("yolov4-obj_cone_tracking.weights"));
  declare_parameter("weights_path", std::string("/default"));

  declare_parameter("yolo_model.config_file.name", std::string("yolov4-obj.cfg"));
  declare_parameter("config_path", std::string("/default"));

  // declare_parameter("subscribers.camera_reading.topic", std::string("concated_cam"));
  declare_parameter("subscribers.camera_reading.topic", std::string("camera1/image_raw"));
...
void YoloObjectDetector::init()
{
  ...
  // std::string cameraTopicName = "concated_cam";
  std::string cameraTopicName = "camera1/image_raw";
  int cameraQueueSize = 10;
  ...
  // imageSubscriber_ = it_->subscribe("concated_cam", 10,
  //   std::bind(&YoloObjectDetector::cameraCallback, this, _1));
  
  imageSubscriber_ = it_->subscribe("camera1/image_raw", 10,
    std::bind(&YoloObjectDetector::cameraCallback, this, _1));
